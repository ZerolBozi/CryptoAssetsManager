...
class WebSocketService:
    ...
    async def initialize(self):
        ...
        file_path = os.path.join(
            root_dir, "app", "services", "symbol_exchange_mapping.json"
        )
        ...
        for exchange_name, exchange_class in exchange_classes.items():
            if (
                exchange_name in self.mapping_data and self.mapping_data[exchange_name]
            ):  # Check if exchange has symbols
    ...
    async def _watch_ohlcv(
        self, exchange_name: str, exchange: ccxtpro.Exchange, symbol: str
    ):
    ...
    async def broadcast(self, message: dict):
        ...
        for (
            client
        ) in self.clients.copy():  # Use copy to avoid modification during iteration
            try:
